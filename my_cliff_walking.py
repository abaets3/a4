...
class MyCliffWalkingEnv(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, size=12, hazard_reward=0, goal_reward=0, normal_reward=0):

        self.spec = gym.envs.registration.EnvSpec(
            id="MyCliffWalking", entry_point="my_cliff_walking:MyCliffWalkingEnv"
        )
    ...
    def _calculate_transition_prob(self, current, delta):
        ...
        if self._cliff[tuple(new_position)]:
            return [(1.0, self.start_state_index, self.hazard_reward, False)]

        terminal_state = (self.shape[0] - 1, self.shape[1] - 1)
        is_terminated = tuple(new_position) == terminal_state

        if is_terminated:
            return [(1.0, new_state, self.goal_reward, is_terminated)]

        return [(1.0, new_state, self.normal_reward, is_terminated)]
...
register(
    id="MyCliffWalking",
    entry_point="my_cliff_walking:MyCliffWalkingEnv",
)