...
class MyTaxiEnv(Env):
    ...
    * v0: Initial versions release
    ...
    def __init__(self, render_mode: Optional[str] = None, size=4):

        self.spec = gym.envs.registration.EnvSpec(
            id="MyTaxi", entry_point="my_taxi:MyTaxi"
        )

        self.desc = np.asarray(MAP, dtype="c")
        self.size = size

        self.locs = locs = [(0, 0), (0, self.size-1), (self.size-1, 0), (self.size-1, self.size-2)]
        self.locs_colors = [(255, 0, 0), (0, 255, 0), (255, 255, 0), (0, 0, 255)]

        num_states = self.size*self.size*5*4
        num_rows = self.size
    ...
    def action_mask(self, state: int):
        ...
        if taxi_col > 0:# and self.desc[taxi_row + 1, 2 * taxi_col] == b":":
            mask[3] = 1
        if pass_loc < self.size-1 and (taxi_row, taxi_col) == self.locs[pass_loc]:
            mask[4] = 1
        if pass_loc == self.size-1 and (
